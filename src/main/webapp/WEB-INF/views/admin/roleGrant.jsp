        	    	...
        	    	if(childList.length>0){
        	    		var checkedTrue = function(){
        	    			$.each(childList,function(index,currentValue){	
        	    				if (resourceTree.tree('find', currentValue.id)) {
                                    resourceTree.tree('check', resourceTree.tree('find', currentValue.id).target);
                                }
        	    			});
        	    		};
        				var checkedFalse = function(){
        	    			$.each(childList,function(index,currentValue){	
        	    				if (resourceTree.tree('find', currentValue.id)) {
                                    resourceTree.tree('uncheck', resourceTree.tree('find', currentValue.id).target);
                                }
        	    			});
        				};
