<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ include file="/commons/global.jsp" %>
<script type="text/javascript">
    $(function() {
        $('#dangerPointsInfoEditForm').form({
            url : '${path}/dangerPointsInfo/edit',
            onSubmit : function() {
                progressLoad();
                var isValid = $(this).form('validate');
                if (!isValid) {
                    progressClose();
                }
                if(isValid){
                	var tree = $('#dangerPointsInfoEditQy').combotree('tree');
                	if(tree.tree('getSelected')==null&&$('#dangerPointsInfoEditQy').combotree('getValue')!=""){
                		$('#dangerPointsInfoEditQy').combotree('setValue','${dangerPointsInfo.dscd}');
                	}
                }
                return isValid;
            },
            success : function(result) {
                progressClose();
                result = $.parseJSON(result);
                if (result.success) {
                    parent.$.modalDialog.openner_dataGrid.datagrid('reload');//之所以能在这里调用到parent.$.modalDialog.openner_dataGrid这个对象，是因为user.jsp页面预定义好了
                    parent.$.modalDialog.handler.dialog('close');
                } else {
                    parent.$.messager.alert('错误', result.msg, 'error');
                }
            }
        });
        
       $('#dangerPointsInfoEditQy').combotree({
    	    url : '${path }/user/tree',
            parentField : 'pid',
            valueField: 'id',
            lines : true,
            width:200,
            panelHeight : 300,
            value : '${dangerPointsInfo.twn}',
            onClick: function(node){
        		$("#dangerPointsInfoEditZwQy").val(node.text);
        	}
        });
        
    });
</script>
<div class="easyui-layout" data-options="fit:true,border:false">
    <div data-options="region:'center',border:false" title="" style="overflow: hidden;padding: 3px;">
        <form id="dangerPointsInfoEditForm" method="post">
            <table class="grid">
            	<tr>
                    <td>所在镇（街）</td>
                    <td>
                    <select id="dangerPointsInfoEditQy" name="dscd"  style="width: 260px; height: 29px;" class="easyui-validatebox" data-options="required:true"></select>
                    <input name="twn" id="dangerPointsInfoEditZwQy" type="hidden" value="${dangerPointsInfo.twn}" ></td>
                </tr>
                <tr>
                    <td>位置</td>
                    <td><input name="id"  type="hidden"  value="${dangerPointsInfo.id}">
                    <input name="add" style="height: 29px;width:260px;" class="easyui-textbox" type="text" data-options="required:true" value="${dangerPointsInfo.add}"></td>
                </tr>
                <tr>
                    <td>隐患点名称</td>
                    <td><input name="vl" style="height: 29px;width:260px;" class="easyui-textbox" type="text" data-options="required:true" value="${dangerPointsInfo.vl}"></td>
                </tr>
                <tr>
                    <td>风险等级</td>
                    <td><input name="dssc" style="height: 29px;width:260px;" class="easyui-textbox" type="text" value="${dangerPointsInfo.dssc}"></td>
                </tr>
                <tr>
                    <td>潜在经济损失（万元）</td>
                    <td><input name="lsec" style="height: 29px;width:260px;" class="easyui-textbox" type="text" value="${dangerPointsInfo.lsec}"></td>
                </tr>
                <tr>
                    <td>安全受威胁人数</td>
                    <td><input name="thpp" style="height: 29px;width:260px;" class="easyui-numberbox" data-options="min:0" type="text" value="${dangerPointsInfo.thpp}"></td>
                </tr>
                <tr>
                    <td>重要设施、厂矿企业</td>
                    <td><input name="imfc" style="height: 29px;width:260px;" class="easyui-textbox" type="text" value="${dangerPointsInfo.imfc}"></td>
                </tr>
                <tr>
                    <td>经度</td>
                    <td><input id="dangerPointsInfoEditJd" name="lgtd" style="height: 29px;width:260px;" class="easyui-numberbox" data-options="min:0,precision:5" type="text" value="${dangerPointsInfo.lgtd}"><a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'fi-magnifying-glass',plain:true" onclick="selMap('dangerPointsInfoEditJd','dangerPointsInfoEditWd');">查看地图</a></td>
                </tr>
                <tr>
                    <td>纬度</td>
                    <td><input id="dangerPointsInfoEditWd" name="lttd" style="height: 29px;width:260px;" class="easyui-numberbox" data-options="min:0,precision:5" type="text" value="${dangerPointsInfo.lttd}"></td>
                </tr>
                
            </table>
        </form>
    </div>
</div>